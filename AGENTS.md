...
EU-HZ/
  apps/
    api/
      cmd/api/              # single Go package; entry point, DB access, ingest, tests
        main.go             # HTTP server and CLI mode switch
        db.go               # store methods, SQL migrations/seeds wiring, query helpers
        ingest.go           # RSS fetch, normalization, heat aggregation, geo catalog
        config.go           # env-backed runtime config
        ingest_test.go      # colocated Go tests
  ...
  db/
    ...
    seeds/                 # SQL seed data
  infra/
    docker-compose.yml     # PostGIS container only
  Makefile                 # root shortcuts for backend run/migrate/seed/docker
...
**api** (`apps/api/`) — Go. Serves `/health`, `/v1/heat`, `/v1/reports`, `/v1/places`, and `/v1/incidents/*`, and also exposes CLI modes for migration, seeding, polling, cleanup, and aggregation. Entry point: `apps/api/cmd/api/main.go`. Package manager: Go modules.
...
go run ./cmd/api poll-once
go run ./cmd/api poll-loop
...
import type { FilterState, PlaceItem, ViewportBounds } from "@/features/api/types";
```
...
- SQL migration and seed files use zero-padded numeric prefixes: `001_init.sql`, `003_heat_cell_dominant_event_type.sql`, `001_seed.sql`.
...
- Functions use verb-led lowerCamelCase: `loadConfig`, `pollOnce`, `aggregateHeat`, `searchPlaces`, `normalizeRange`.
...
- React components and shared types use PascalCase: `MapShell`, `SearchBar`, `ReportsPanel`, `IncidentModal`, `FilterState`, `PlaceItem`.
...
```go
for {
	if err := pollOnce(ctx, db, slug); err != nil {
		log.Printf("poll-loop error: %v", err)
	}
...
```go
func TestNormalizeIncidentFindsCityAndEvent(t *testing.T) {
	source := sourceRecord{Slug: "bbc-world", Type: "news", TrustWeight: 0.58}
	item := rawSourceItem{
		Title:       "Police investigate harassment reports in Brussels nightlife district",
		Text:        "Police investigate harassment reports in Brussels nightlife district after several women were attacked late at night.",
		PublishedAt: time.Date(2026, 6, 30, 8, 0, 0, 0, time.UTC),
	}

	incident, ok := normalizeIncident(item, source)
	if !ok {
...
This clone has no commit history yet, so no commit-message format, branch naming pattern, or merge strategy can be extracted from the repo itself. Do not claim a git convention that is not present in history.

Because there is no evidence, keep any future git workflow notes outside this file until the repository has real history.
...
- There is no repo README or LICENSE file at the time of extraction.
- There is no `.github/workflows/` directory in this repo snapshot, so do not assume CI exists.

### API (`apps/api`)

- Go version is pinned in `apps/api/go.mod`.
...
- Do not claim formatter, CI, README, license, or git conventions that are not present in the repo.
